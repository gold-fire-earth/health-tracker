...
你可能看到 IP 地址长这样 `185.199.108.153`，4 段数字，每段 0-255，排列组合确实有限（IPv4 总共约 43 亿个，早就分完了）。但**域名不是 IP 地址**：

```
IP 地址的样子：  185.199.108.153        ← 数字，数量有限
域名的样子：     keephealthy-wxl.top     ← 字母+数字，数量几乎无限
```

域名的每一位可以是 a-z、0-9、横杠，排列起来是天文数字。比如 20 个字符的组合就是 37²⁰ ≈ 10³¹ 种可能。

**域名不会重名**，因为它是全球统一注册的，先到先得。你注册了 `keephealthy-wxl.top`，地球上就不可能有第二个人再注册它——就像手机号，一个号码只能属于一个人。

### 1.7 一个 IP 怎么挂几百万个域名？

这是新手最想不通的地方：IP 数量有限，但网站数量多得多，怎么做到的？

答案：**一个 IP 可以同时托管几百万个域名**。GitHub 的 `185.199.108.153` 这一个 IP 上同时跑着几十万个网站。

区别它们的方法是浏览器请求里的 `Host` 头：

```
用户A → 浏览器偷偷带上: "我要访问 keephealthy-wxl.top"
用户B → 浏览器偷偷带上: "我要访问 myblog.github.io"

两个请求到了同一个 IP: 185.199.108.153
         │
         ▼
GitHub 服务器一看 Host 头：
  "keephealthy-wxl.top → 去这个目录取文件"
  "myblog.github.io   → 去那个目录取文件"
```
...
你可能想问：既然 IP 是有限的，我的手机每次上网怎么也有个 IP？

这就是 **DHCP（动态主机配置协议）** 的作用。运营商（电信/联通/移动）手里有一批公网 IP，但不会给每个用户固定分配一个——而是谁上网就临时借一个，断网了就回收给别人用。

再加上家里的 WiFi 路由器用 **NAT（网络地址转换）** 技术，把 1 个公网 IP 分给家里所有设备（手机、电脑、平板共享），大大减缓了 IPv4 用尽的速度。
